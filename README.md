...
常见的单链表是邻接表，用来存储树和图。我们用数组模拟单链表，`e[N]`表示值，`ne[N]`表示next。

另外与两个变量：head表示头结点的下标，idx表示当前用到了哪个节点。
...
括号只会出现在中缀表达式中。例如`(2+2)*(1+1)`，用前缀表达式是`* + 2 2 + 1 1`，后缀表达式是`2 2 + 1 1 + *`

**利用栈，达到中缀表达式求值中序递归的效果**。

中序遍历的时候，先遍历左子树，然后遍历本节点，发现**一定是一个运算符**，然后去遍历右子树。

#### 判断某棵子树被遍历完

只要往上走，就说明遍历完了，往下走表示没遍历完。若当前运算符优先级小于等于上一个运算符优先级，说明子树遍历完了，向上走。

#### 括号的处理

遇到右括号的时候，开始向左递归，直到遇到上一个左括号。
